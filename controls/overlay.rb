include_controls 'microsoft-windows-server-2016-stig-baseline' do
  ...
  control 'V-73283' do
    ...
    If temporary accounts are used, the operating system must be configured to automatically terminate these types of accounts after a CMS-defined time period of 72 hours.
  ...
  control 'V-73307' do
    ...
    If the system needs to be configured to an NTP server, configure the system to point to an authorized time server by setting the policy value for Computer Configuration >> Administrative Templates >> System >> Windows Time Service >> Time Providers >> \"Configure Windows NTP Client\" to \"Enabled\", and
  ...
  control 'V-73311' do
    title "Windows Server 2016 must have the number of allowed bad logon attempts configured to five or less."
  ...
  control 'V-73315' do
    title "Windows Server 2016 password history must be configured to 6 passwords remembered."
  ...
  control 'V-73605' do
    ...
    desc "To ensure secure CMS websites and CMS-signed code are properly validated, the system must trust the CMS Root Certificate Authorities (CAs). The CMS root certificates will ensure that the trust chain is established for server certificates issued from the CMS CAs."
  ...
  control 'V-73613' do
    ...
    There are multiple sources from which lists of valid CMS CAs and approved ECAs can be obtained:
    The Global Directory Service (GDS) website provides an online source. The address for this site is https://crl.gds.disa.mil.
    
    CMS Public Key Enablement (PKE) Engineering Support maintains the InstallRoot utility to manage CMS supported root certificates on Windows computers, which includes a list of authorized CAs. The utility package can be downloaded from the PKI and PKE Tools page on IASE:
  ...
  control 'V-73615' do
    impact 0.0
    title "PKI certificates associated with user accounts must be issued by the CMS PKI or an approved External Certificate Authority (ECA)."
    desc 'caveat', 'Not applicable for this CMS ARS 3.1 overlay, since the related security control is not applied to this system categorization in CMS ARS 3.1'
  end
